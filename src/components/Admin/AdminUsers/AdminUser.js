    ...
    const handleOnChangeAccount = async (user) => {
        if (user.isBlock === true) {
            if (window.confirm("Are you sure you want to open this account")) {
                await axios.post(`https://facemaskufl.herokuapp.com/user/block/${user._id}`, { isBlock: !user.isBlock }, {
    ...
    useEffect(() => {
        ...
        if (token) {
            const getAllUser = async () => {
                try {
                    const res = await axios.get('https://facemaskufl.herokuapp.com/user/users', {
                        headers: { Authorization: token }
                    });
